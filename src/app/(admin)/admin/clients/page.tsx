...
export const metadata: Metadata = {
  title: `${PRODUCT_NAME} Admin — Clients`,
};
...
export default async function AdminClientsPage({
  ...
  return (
    <AdminPageShell
      icon={Briefcase}
      title="Clients"
      description="Every account on the platform — managed custom jobs and self-serve SaaS."
      actions={<NewClientDialog />}
    >
      <div className="flex flex-wrap gap-2">
        {FILTER_TABS.map(({ value, label }) => {
          const active = filter === value;
          const href = value === "all" ? "/admin/clients" : `/admin/clients?type=${value}`;
          return (
            <Link
              key={value}
              href={href}
      ...
      {loadError ? (
        ...
        <AdminSectionCard
          title="All clients"
          description={`${rows.length} client${rows.length === 1 ? "" : "s"}${filter !== "all" ? ` (${clientProvisionSourceLabel(filter)})` : ""}.`}
        >
                ...
                {rows.length === 0 ? (
                  <tr>
                    ...
                    >
                      ...
                      to provision the first managed client.
                    </td>
                  </tr>
                ) : (
                  rows.map((row) => (
                    <tr key={row.orgId} className="hover:bg-gray-50/50">
                      ...
                      <td className="whitespace-nowrap px-5 py-4 text-right">
                        <TenantRowActions
                          ...
                          clientHref={`/admin/clients/${row.orgId}`}
                        />
                      </td>
                    </tr>
                  ))
