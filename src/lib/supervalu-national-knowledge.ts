...
export function buildSupervaluNationalKnowledgeSection(): string {
  return [
    ...
    "Real Rewards is SuperValu's free loyalty programme in Ireland. Shoppers can register in store (pick up a card) or via the Real Rewards app on the App Store or Google Play.",
    "",
    "**Spoken web addresses:** never read raw URLs aloud. Say **SuperValu dot ie slash rewards** (not supervalu.ie/rewards). Offer sendSms if they want the link texted.",
    "",
    ...
    "• Webchat: SuperValu dot ie slash rewards (Chat with us)",
    "",
